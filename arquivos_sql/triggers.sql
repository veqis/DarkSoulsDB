...
RETURNS TABLE (personagem_nome VARCHAR, alvo_nome VARCHAR, resultado VARCHAR) AS $$
DECLARE
	item_d INT;
    pd_item DECIMAL;
    alvo_row dropa%ROWTYPE;
BEGIN
    ...
    LOOP
        ...
        FROM dropa
        WHERE nome_entidade = alvo_row.nome_entidade;

        IF random_between(1, 100) <= pd_item * (item_d / 100.0) THEN
